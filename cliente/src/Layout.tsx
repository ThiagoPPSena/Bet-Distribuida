import { AppBar, Box, Drawer, IconButton, List, ListItem, ListItemButton, ListItemText, MenuItem, Select, SelectChangeEvent, Toolbar, Typography } from "@mui/material"
...
function Layout() {
  ...
  const DrawerList = (
    <Box sx={{ width: '10em', height: '100vh', backgroundColor: '#5A157F', color: "white"}} role="presentation" onClick={toggleDrawer(false)}>
      <List>
  ...
  return (
    ...
    <Box sx={{
      ...
      <AppBar position="static" sx={{backgroundColor: '#5A157F'}} elevation={0}>
        <Toolbar>
          {/* Ícone de menu */}
          <IconButton
            size="large"
          ...
          {/* Botões de navegação */}
            <Typography variant="h6" color="inherit" sx={{ mr: 2, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              <Wallet sx={{mr: 1}}/>
              {balanceEth} ETH
            </Typography>
            <Select
            ...
            {accounts.map((account, index) => (
              <MenuItem key={index} value={account}>{`Conta ${index+1}`} </MenuItem>
              ))}    
            </Select>
